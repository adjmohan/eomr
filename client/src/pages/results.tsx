...
const ResultsPage = () => {
  ...
  const fetchResults = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:3000/api/results/${batchCode}`);
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'Failed to fetch results';
      setError(`Error: ${errorMessage}. ${error.response?.data?.details || ''}`);
